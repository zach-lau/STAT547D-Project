functions {
    ...
    real gp_variance(vector k, matrix Kinv, real sf2){
        return sf2 - dot_product(k, Kinv*k);
    }
    real logf(vector x, array[] vector alpha, matrix Kinv, vector weights, vector l, real sf2, real mu, real s2){
        // Evaluate surogate likelihood using gp mean
        int m = size(alpha);
        vector[m] k = cross(x, alpha, l, sf2);
        real postmean = gp_mean(k, weights)*sqrt(s2)+mu;
        real postvar =  gp_variance(k, Kinv, sf2)*s2;
        return postmean + 0.5*postvar;
    }
}
...
model {
    theta ~ normal(0, 1);
    target += logf(theta, alpha, Kinv, weights, l, sf2, mu, s2);
}
